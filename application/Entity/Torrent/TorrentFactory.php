...
class TorrentFactory
{
    ...
    {
        $self = \Rid\Helpers\ContainerHelper::getContainer()->get('pdo')->prepare('SELECT * FROM `torrents` WHERE id=:id LIMIT 1;')->bindParams([
            'id' => $tid
        ])->queryOne();
    ...
    {
        $fetch = \Rid\Helpers\ContainerHelper::getContainer()->get('pdo')->prepare([
