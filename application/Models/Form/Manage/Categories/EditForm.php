...
class EditForm extends Validator
{
    ...
    protected function checkCategoryData()
    {
        ...
        $parent_cat_fpath = container()->get('pdo')->prepare('SELECT `full_path` FROM `categories` WHERE `id` = :pid')->bindParams([
            'pid' => $cat_parent_id
        ])->queryScalar();
        ...
        if ($cat_id !== 0) {  // Check if old links should be update
            $this->cat_old_data = container()->get('pdo')->prepare('SELECT * FROM `categories` WHERE id = :id')->bindParams([
                'id' => $cat_id
            ])->queryOne();
        ...
        if ($flag_check_full_path) {  // Check if full path key is duplicate or not.
            $check_full_path = container()->get('pdo')->prepare('SELECT COUNT(`id`) FROM `categories` WHERE `full_path` = :fpath')->bindParams([
                'fpath' => $full_path
            ])->queryScalar();
    ...
    public function flush()
    {
        if ((int)$this->cat_id !== 0) {  // to edit exist cat
            container()->get('pdo')->update('categories', $this->cat_data_diff, [['id', '=', $this->cat_id]])->execute();
            if ($this->cat_parent_id !== 0) {  // Disabled post to parent cat
                container()->get('pdo')->prepare('UPDATE `categories` SET `enabled` = 0 WHERE `id` = :pid')->bindParams([
                    'pid' => $this->cat_parent_id
                ])->execute();
            }
            // TODO Add site log
        } else {  // to new a cat
            container()->get('pdo')->insert('categories', $this->cat_new_data)->execute();
            // TODO Add site log
        }
